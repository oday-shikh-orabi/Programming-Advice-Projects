...
enum enMainMenueOptions { eShowClientList = 1, eAddNewClient = 2, eDeleteClient = 3, eUpdetClientInfo = 4, eFindClilent = 5,eTransactions = 6 ,eExit = 7 };
...
bool FindClientByAccountNumber(string AccountNumber, vector<sClient> vClients, sClient& Client ,double& Balance)
{
	for (sClient C : vClients)
	{
		if (C.AccountNumber == AccountNumber)
...
void Withdraw(vector<sClient>& vClients)
{
	...
	double WithdrawNumber , AccountBalances;
	...
	do
	{
		...
		if (FindClientByAccountNumber(AccountNumber, vClients, Client,AccountBalances))
		{
			PrintClientCard(Client);
			...
			while (AccountBalances < WithdrawNumber)
			{
				cout << "\nAmount Exceeds the balance, you can withdraw up to :" << AccountBalances;
...
void ShowMainMenueScreen()
{
	...
	do
	{
		...
		else if (enAnswer == enMainMenueOptions::eExit)
		{
			system("cls");
	...
	} while (enAnswer != enMainMenueOptions::eExit);
	system("pause");
