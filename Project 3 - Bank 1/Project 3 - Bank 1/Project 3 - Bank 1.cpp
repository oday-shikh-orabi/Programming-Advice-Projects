...
enum enMainMenue { ShowClientList = 1, AddNewClient = 2 , DeleteClient = 3 , UpdetClientInfo = 4 , FindClilent = 5 , Exit = 6 };
...
void AddNew_Client(vector<sClient>& vClients , string AccountNumber)
{
...
void AddClients(vector<sClient>& vClients)
{
	...
	do
	{
		...
		while (FindClientByAccountNumber(AccountNumber, vClients))
		{
			...
			cout << "Client With [" << AccountNumber << "] already exists , Enter Anothe Accout Number? ";
			getline(cin >> ws, AccountNumber);
		}


		
		AddNew_Client(vClients, AccountNumber);
...
enMainMenue ConvertintToEnum(short Answer)
{
	switch (Answer)
	{
	...
	case enMainMenue::UpdetClientInfo:
	{
		return UpdetClientInfo;
		break;
	}
	case enMainMenue::FindClilent:
	{
		return FindClilent;
		break;
	}
...
void ShowMainMenueScreen()
{
	...
	do
	{
		...
		enAnswer = ConvertintToEnum(ReadUserChoiceInMainMenue());
		...
		else if (enAnswer == enMainMenue::Exit)
		{
			UserChoeseFromMainMenue(enAnswer,vClients);
	...
	} while (enAnswer != enMainMenue::Exit);
